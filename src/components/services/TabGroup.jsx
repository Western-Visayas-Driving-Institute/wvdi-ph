...
const TabGroup = ({ defaultTab, children }) => {
  ...
  return (
    <div className="w-full">
      <div role="tablist" aria-label="Service Tabs" className="flex flex-wrap md:flex-nowrap border-b border-[#F57C00]/30 mb-4">
        {tabs.map(tab => (
          <button
            key={tab.props.label}
            role="tab"
            ...
            className={`px-4 py-2 text-lg font-semibold transition-colors duration-200 border-b-2 ${activeTab === tab.props.label ? 'border-[#F57C00] text-[#F57C00] bg-white' : 'border-transparent text-[#333] bg-transparent hover:bg-[#F57C00]/10'}`}
